...
fn main() {
    ...
    let xx= still_unmoved.as_mut();
    
    let xx = unsafe{Pin::get_unchecked_mut(xx)};
    xx.data = "xxxxx".to_owned();

    // unsafe{println!("{}", still_unmoved.slice.as_ref())};


    let mut xxx = Unmovable {
        data: "old".to_owned(),
        slice: NonNull::dangling(),
        _pin: PhantomPinned,
    };
    std::mem::swap(&mut xxx, xx);

    xxx.data = "new data".to_owned();
    unsafe {println!("{}", xxx.slice.as_ref())}
